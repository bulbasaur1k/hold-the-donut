...
| **4b. Chain + fragment в Vision-путь** | ⏭️ сделать `vision_server_splice`/`tls_plain_relay` дженериками по upstream + прокинуть chain/fragment в `handle_xray_vision_session` (чтобы REALITY был входом каскада). | средний |
