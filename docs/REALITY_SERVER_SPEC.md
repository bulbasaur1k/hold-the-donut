...
| **4b. Chain в Vision-путь** | ✅ **DONE 2026-06-25** — `vision_server_splice`/`tls_plain_relay` дженерики по upstream (`G: AsyncRead+AsyncWrite+Unpin+Send`); `handle_xray_vision_session` диспетчит chain/freedom (Box<dyn Duplex>); `run_reality_proxy` несёт `outbounds`. E2e `reality_cascade_e2e.rs`: REALITY-вход → chain → veil-выход → echo. 152 теста. ⏭️ fragment в Vision-freedom (YouTube на REALITY-входе) — follow-on (сложно в splice; пока zapret-сайдкар). | done |
